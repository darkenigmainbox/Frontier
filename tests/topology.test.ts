...
describe('xeric plants keep their body topology and organ silhouettes', () => {
  for (const preset of PRESETS.filter(isSucculent)) {
    it(`${preset.name} is a closed cactus or succulent specimen`, () => {
      const r = generateTree(cloneParams(preset));
      expect(r.report.closed, 'body has no boundary edges').toBe(true);
      expect(r.report.manifold, 'body is 2-manifold').toBe(true);
      expect(r.report.components, 'body is one connected piece').toBe(1);
      expect(r.report.eulerCharacteristic, 'body is genus 0').toBe(2);
      expect(r.report.quadRatio, 'body keeps quad flow').toBeGreaterThan(0.99);
      expect(r.summary.height).toBeGreaterThan(0.1);
      if (preset.succulent!.rosetteCount > 0) expect(r.leaves.count).toBe(preset.succulent!.rosetteCount);
      // No generated organ may carry an invalid wind attribute; this catches
      // malformed cards before they reach the viewport shader/exporter.
      for (const v of r.leaves.wind) expect(v).toBeGreaterThanOrEqual(0);
      for (const v of r.leaves.wind) expect(v).toBeLessThanOrEqual(1);
    });
  }
});
