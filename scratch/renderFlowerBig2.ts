...
const W = 480, H = 480;
function renderTop(mesh: QuadMesh, elevDeg: number): Uint8Array {
  ...
  for (let i=0;i<n;i++){
    ...
    if(px[i]<minX)minX=px[i]; if(px[i]>maxX)maxX=px[i]; if(py[i]<minYc)minYc=py[i]; if(py[i]>maxYc)maxYc=py[i];
  }
  const spanX=Math.max(1e-6,maxX-minX), spanY=Math.max(1e-6,maxYc-minYc);
  const sc=Math.min((W-30)/spanX,(H-30)/spanY);
  const ox=(W-sc*spanX)/2-sc*minX, oy=(H-sc*spanY)/2+sc*maxYc;
...
const g = { ...DEFAULT_DESERT, ...preset.desert, spinesPer: 0, centralSpine: 0, spineDensity: 0, flowerLength: 0.075, flowerRadius: 0.028 };
const built = new DesertMesher(g, 1).build();
writeFileSync('renders/golden_barrel_flowers_big.bmp', renderTop(built.mesh, 55));
console.log('done', JSON.stringify(built.stats));
