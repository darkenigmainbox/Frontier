/**
 * Top-down / oblique-top headless preview for desert plants — used to inspect
 * barrel-cactus crowns. Same software rasteriser as scripts/desertRender.ts
 * but the camera looks down from above at a steep angle.
 *
 * Usage: npx vite-node scratch/renderTop.ts [name-filter] [seed]
 */
// @ts-ignore
...
function render(mesh: QuadMesh, elevDeg: number, topFrac: number): Uint8Array {
  const p = mesh.positions;
  const n = mesh.vertexCount;
  let maxY = 0.001;
  for (let i = 0; i < n; i++) if (p[i * 3 + 1] > maxY) maxY = p[i * 3 + 1];
  const yCut = maxY * (1 - topFrac);

  const elev = (elevDeg * Math.PI) / 180;
  // Camera looks down at `elev` above horizontal, from +Z rotated slightly.
  const cy = Math.sin(elev);
  const cz = Math.cos(elev);
  // Build camera basis: forward = (0,-cy,-cz) roughly looking down+forward.
  const fwd = { x: 0, y: -cy, z: -cz };
  const upWorld = { x: 0, y: 0, z: 1 };
  // right = fwd x up
  let rx = fwd.y * upWorld.z - fwd.z * upWorld.y;
  let ry = fwd.z * upWorld.x - fwd.x * upWorld.z;
  let rz = fwd.x * upWorld.y - fwd.y * upWorld.x;
  ...
  let minX = Infinity, maxX = -Infinity, minYc = Infinity, maxYc = -Infinity;
  for (let i = 0; i < n; i++) {
    const x = p[i * 3], y = p[i * 3 + 1], z = p[i * 3 + 2];
    if (y < yCut - 0.02) continue; // only frame near the crown
    const sx = x * rx + y * ry + z * rz;
    const sy = x * ux + y * uy + z * uz;
    const sz = x * fwd.x + y * fwd.y + z * fwd.z;
    px[i] = sx; py[i] = sy; pz[i] = sz;
    if (sx < minX) minX = sx;
    if (sx > maxX) maxX = sx;
    if (sy < minYc) minYc = sy;
    if (sy > maxYc) maxYc = sy;
  }
  if (!Number.isFinite(minX)) { minX = -1; maxX = 1; minYc = -1; maxYc = 1; }
  // fill in the rest (below-cut) too, projected, so faces occluding from below still z-test correctly
  for (let i = 0; i < n; i++) {
    const x = p[i * 3], y = p[i * 3 + 1], z = p[i * 3 + 2];
    const sx = x * rx + y * ry + z * rz;
    const sy = x * ux + y * uy + z * uz;
  ...
  const sc = Math.min((W - 30) / spanX, (H - 30) / spanY) * 1.0;
  const ox = (W - sc * spanX) / 2 - sc * minX;
  const oy = (H - sc * spanY) / 2 + sc * maxYc;
  const sx2 = new Float64Array(n);
  const sy2 = new Float64Array(n);
  for (let i = 0; i < n; i++) {
    sx2[i] = ox + sc * px[i];
    sy2[i] = oy - sc * py[i];
  }
  ...
  const cols = [[46, 110, 58], [74, 148, 74], [216, 196, 110]];
  const tri = (a: number, b: number, c: number, lvl: number): void => {
    ...
    const cnx = nx * rx + ny * ry + nz * rz;
    const cny = nx * ux + ny * uy + nz * uz;
    const cnz = nx * fwd.x + ny * fwd.y + nz * fwd.z;
    if (cnz <= 0) return;
...
for (const preset of DESERT_PRESETS) {
  ...
  const buf = render(built.mesh, 62, 0.35);
  const file = `renders/${preset.name.toLowerCase().replace(/[^a-z0-9]+/g, '_')}_top_s${seed}.bmp`;
