...
function renderTop(mesh: QuadMesh, elevDeg: number, topFrac: number): Uint8Array {
  ...
  for (let i=0;i<n;i++){
    ...
    if(sx<minX)minX=sx; if(sx>maxX)maxX=sx; if(sy<minYc)minYc=sy; if(sy>maxYc)maxYc=sy;
  }
...
writeFileSync('renders/golden_barrel_closeup.bmp', renderTop(built.mesh, 45, 0.22));
console.log('done');
