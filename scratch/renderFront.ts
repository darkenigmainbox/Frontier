// Front-ish view but closer, more elevated for rosettes
...
const elev=Number(argv[4]??18)||18;
...
for (const preset of DESERT_PRESETS) {
  ...
  const buf = render(built.mesh, elev);
  const file = `renders/${preset.name.toLowerCase().replace(/[^a-z0-9]+/g,'_')}_front_s${seed}.bmp`;
  writeFileSync(file, buf);
  console.log(`${preset.name}: ${file}`);
}
