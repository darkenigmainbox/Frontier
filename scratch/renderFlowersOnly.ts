...
const W = 480, H = 480;
function renderTop(mesh: QuadMesh, elevDeg: number, topFrac: number): Uint8Array {
  ...
  for (let i=0;i<n;i++){
    const y=p[i*3+1];
    if (y < maxY*(1-topFrac)-0.02) continue;
...
writeFileSync('renders/golden_barrel_flowers_top.bmp', renderTop(built.mesh, 62, 0.4));
console.log('done');
