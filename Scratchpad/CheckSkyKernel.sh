...
printf '%s' "$Code" | grep -q 'AnalyticFogAlong(CameraOrigin, surface.xyz, accumulatedRadiance)'
Report $? "height and aerial fog cross the GI-on surface segment"
printf '%s' "$SkyCode" | grep -q 'float HeightTau = 0.0'
Report $? "the shader integrates height fog analytically"
printf '%s' "$SkyCode" | grep -q 'SkyFogAerial'
Report $? "the fog controls arrive through the sky record"
...
Report $? "the sky, weather and fog record is pinned at 368 bytes"
...
grep -q 'PackSkyFog' "$PackSite"
Report $? "the Project Zero sequence packs height and aerial fog"
grep -q 'Settings.Fog.HeightEnabled' "$PackSite"
Report $? "the raster receives fog visibility from the outliner"
