...
//        offset 320   SkyFogHeight         vec4
//        offset 336   SkyFogColour         vec4
//        offset 352   SkyFogAerial         vec4
//        block size = 368 B
//
...
struct SkyConstantRecord
{
    ...
    float    FogHeight[4];      // density, falloff height, sun scatter, enabled
    float    FogColour[4];      // rgb height-fog colour, unused
    float    FogAerial[4];      // density, start distance, Mie blend, enabled
};
...
static_assert(offsetof(SkyConstantRecord, FogHeight)       == 320u, "SkyFogHeight sits at offset 320");
static_assert(offsetof(SkyConstantRecord, FogColour)       == 336u, "SkyFogColour sits at offset 336");
static_assert(offsetof(SkyConstantRecord, FogAerial)       == 352u, "SkyFogAerial sits at offset 352");
...
// Packs the two non-box fog entities into the tail rows. Height fog is an analytic finite-segment medium and
// aerial fog is the distance-dependent atmospheric veil; keeping their controls in the same record prevents the
// GPU miss, bounce and surface paths from observing different panel edits. The enabled lanes are explicit because
// a hidden outliner row must disable the effect without erasing its last slider values.
inline void PackSkyFog(SkyConstantRecord& R, const FogSettings& Fog, bool HeightVisible,
                       bool AerialVisible) noexcept
{
    const bool UseHeight = HeightVisible && Fog.HeightEnabled;
    const bool UseAerial = AerialVisible && Fog.AerialEnabled;
    R.FogHeight[0] = Fog.HeightDensity;
    R.FogHeight[1] = Fog.FalloffHeight;
    R.FogHeight[2] = Fog.SunScatter;
    R.FogHeight[3] = UseHeight ? 1.0f : 0.0f;
    for (int C = 0; C < 3; ++C) R.FogColour[C] = Fog.HeightColour[C];
    R.FogAerial[0] = Fog.AerialDensity;
    R.FogAerial[1] = Fog.AerialStart;
    R.FogAerial[2] = Fog.AerialMie;
    R.FogAerial[3] = UseAerial ? 1.0f : 0.0f;
}
