                             ...
                             uint32_t Width, uint32_t Height, unsigned char* Rgba, double& MeanLum) noexcept
{
    ...
    for (uint32_t Y = 0u; Y < Height; ++Y)
    {
        for (uint32_t X = 0u; X < Width; ++X)
        {
            ...
            // The reference panel exposes Height Fog and Atmospheric Fog as separate entities from the local
            // boxes. They are finite-segment media: apply them only to a ray that actually reaches scene geometry,
            // after the cloud/local-volume composite and before the shared transfer. Keeping this here is important
            // for the GI-off path because a UI toggle must change the pixels, not only the inspector sheet.
            if (Celestial_.Enabled && TriId_[Idx] != kMiss
                && (Celestial_.Fog.HeightEnabled || Celestial_.Fog.AerialEnabled))
            {
                const uint32_t Tri = TriId_[Idx];
                const float W0 = Bary_[Idx * 2u], W1 = Bary_[Idx * 2u + 1u], W2 = 1.0f - W0 - W1;
                const float P[3] = { W0 * Flat[Tri].VertexAlphaX + W1 * Flat[Tri].VertexBetaX + W2 * Flat[Tri].VertexGammaX,
                                     W0 * Flat[Tri].VertexAlphaY + W1 * Flat[Tri].VertexBetaY + W2 * Flat[Tri].VertexGammaY,
                                     W0 * Flat[Tri].VertexAlphaZ + W1 * Flat[Tri].VertexBetaZ + W2 * Flat[Tri].VertexGammaZ };
                const float Dx = P[0] - Eye[0], Dy = P[1] - Eye[1], Dz = P[2] - Eye[2];
                const float Distance = std::sqrt(Dx * Dx + Dy * Dy + Dz * Dz);
                if (Distance > 0.0f)
                {
                    const float HeightTau = VolumetricMedia::HeightFogOpticalDepth(
                        Celestial_.Fog, Eye[2], P[2], Distance);
                    const float AerialDistance = std::fmax(Distance - Celestial_.Fog.AerialStart, 0.0f);
                    const float MeanRayleigh = (Celestial_.Medium.RayleighScattering[0]
                                              + Celestial_.Medium.RayleighScattering[1]
                                              + Celestial_.Medium.RayleighScattering[2]) / 3.0f
                                              * Celestial_.Medium.RayleighStrength;
                    const float MeanMie = Celestial_.Medium.MieScattering * Celestial_.Medium.MieStrength;
                    const float AerialTau = Celestial_.Fog.AerialEnabled
                        ? AerialDistance * (MeanRayleigh + MeanMie) * Celestial_.Fog.AerialDensity
                        : 0.0f;
                    const float TotalTau = HeightTau + AerialTau;
                    const float Transmission = std::exp(-std::fmax(TotalTau, 0.0f));
                    const float HeightScatter = 1.0f - std::exp(-std::fmax(HeightTau, 0.0f));
                    const float AerialScatter = 1.0f - std::exp(-std::fmax(AerialTau, 0.0f));
                    for (int C = 0; C < 3; ++C)
                    {
                        const float HeightLight = Celestial_.Fog.HeightColour[C] * Celestial_.Fog.SunScatter;
                        const float AirLight = Mix01(Celestial_.Fog.AerialMie, 1.0f, AerialScatter)
                                             * SkyAmbient[C];
                        Linear[C] = Linear[C] * Transmission + HeightLight * HeightScatter
                                  + AirLight * AerialScatter;
                    }
                }
            }
