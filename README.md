...
a single skin. 30 woody tree species, 5 xeric plants and 14 grasses ship as presets.
...
| Xeric plants | `src/plant/succulentMesher.ts`, `src/plant/succulentParams.ts` | Saguaro and barrel cacti keep ribs in the welded body surface; prickly pear pads, agave blades and aloe leaves are seeded, curved organs with wind data. Five desert presets share the closed-body validation/export path. |
...
Form and topology only: solid-colour materials, proxy leaf cards on trees, and authored waxy/cactus
materials for the xeric family. Bark and blade textures, UV layout for atlases, LOD chains and
fully volumetric spine/areole assets are the next step; the quad flow and UV seams are already laid
out to make that straightforward. Grasses and xeric plants are the first non-tree families;
shrubs and ferns can reuse the same organ/window machinery.
