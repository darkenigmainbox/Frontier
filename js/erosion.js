...
export async function streamPowerErode({
  ...
  for (let it = 0; it < iterations; it++) {
    ...
    // ---- 2) hillslope diffusion (edge-preserving selective conductance) ----
    // On gentle slopes & valley floors (low slope), g -> 1, so alluvium and
    // valley floors smooth out naturally.
    // On steep bedrock cliffs and sharp knife-edge ridges (high slope),
    // g -> 0, so cliff rims and crests stay RAZOR SHARP instead of blurry!
    for (let j = 0; j < N; j++) {
      const row = j * N;
      for (let i = 0; i < N; i++) {
        const c = row + i;
        ...
        let sum = 0, cnt = 0;
        let dhX = 0, dhZ = 0;
        if (i > 0) { sum += h[c - 1]; cnt++; dhX -= h[c - 1]; } else { dhX -= cur; }
        if (i < N - 1) { sum += h[c + 1]; cnt++; dhX += h[c + 1]; } else { dhX += cur; }
        if (j > 0) { sum += h[c - N]; cnt++; dhZ -= h[c - N]; } else { dhZ -= cur; }
        if (j < N - 1) { sum += h[c + N]; cnt++; dhZ += h[c + N]; } else { dhZ += cur; }
        const lap = (sum - cnt * cur) / cnt;
        if (lap > 1e-5 || lap < -1e-5) {
          const sMag = Math.hypot(dhX * 0.5, dhZ * 0.5) / voxel;
          const g = 1 / (1 + (sMag / 0.45) * (sMag / 0.45));
          h[c] = cur + D * g * lap;
        }
      }
    }
    ...
    for (let j = 1; j < N - 1; j++) {
      for (let i = 1; i < N - 1; i++) {
        ...
        if (count < maxCells) {
          ...
          while (guard++ < 150) {
            level -= 0.2 * voxel;
            if (level < seaLevel - 2.5) break; // reached ocean outlet; stop cutting
 ...
 * rounds ridges, matures slopes. Run it between erosion
...
export async function passDiffuse(cfg) {
  ...
  const iters = Math.max(8, Math.round(iterations * 0.4));
  const D = Math.min(0.20, diffusion ?? 0.05);
  for (let it = 0; it < iters; it++) {
    for (let j = 0; j < N; j++) {
      const row = j * N;
      for (let i = 0; i < N; i++) {
...
/**
 * PASS — PARTICLE DROPLETS (Discrete hydraulic erosion).
 * Thousands of discrete physics droplets advect down -∇h with
 * inertia, momentum, and dynamic sediment capacity.
 * Anti-tunneling bedload resistance and depression filling
 * prevent drilling holes into the terrain.
 */
export async function passParticles(cfg) {
  const { h, baseH, N, voxel, seed, seaLevel, particles = 25000, erodibility, cutFraction, maps, yieldControl } = cfg;
  const bedrock = Math.min(seaLevel - 14, -40);
  const res = await particleErode({
    h, baseH, N, voxel, seed: (seed ^ 0x3f1a) >>> 0,
    particles, erodibility, cutFraction, bedrock, seaLevel,
    antiTunneling: true, talusRelax: true,
    maps, yieldControl,
  });
  drainSmallBasins(h, N, voxel, seaLevel);
  fillMicroPits(h, N, voxel);
...
/**
 * PASS — GEOLOGICAL STRATA & TERRACES.
 * Natural sedimentary bedding: alternating strata of resistant
 * caprock (limestone/sandstone) and soft layers (shale/mudstone).
 * Carves crisp stepped cliff benches and horizontal ledges
 * (Badlands / Grand Canyon morphology).
 */
export async function passStrata(cfg) {
  const { h, N, voxel, seaLevel, strataAmp = 0.35, strataFreq = 0.55, maps, yieldControl } = cfg;
  const size = N * N;
  const cellArea = voxel * voxel;
  let carved = 0;
  for (let idx = 0; idx < size; idx++) {
    const cur = h[idx];
    if (cur < seaLevel + 0.5) continue;
    const phase = (cur - seaLevel) * strataFreq;
    const s1 = Math.sin(phase);
    const s2 = Math.sin(phase * 2.0) * 0.30;
    const terrace = s1 + s2;
    const cx = idx % N, cy = (idx / N) | 0;
    let slope = 0;
    if (cx > 0 && cx < N - 1 && cy > 0 && cy < N - 1) {
      const dx = (h[idx + 1] - h[idx - 1]) / (2 * voxel);
      const dz = (h[idx + N] - h[idx - N]) / (2 * voxel);
      slope = Math.hypot(dx, dz);
    }
    if (slope > 0.28 && terrace < 0) {
      const cut = Math.min(-terrace * strataAmp * Math.min(1.4, slope) * voxel * 0.16, 0.22 * voxel);
      h[idx] -= cut;
      if (maps && maps.erosionMap) maps.erosionMap[idx] += cut;
      carved += cut * cellArea;
    }
  }
  drainSmallBasins(h, N, voxel, seaLevel);
  fillMicroPits(h, N, voxel);
  const ff2 = finalFlow(h, N, voxel, 42, seaLevel);
  return { carvedM3: carved, depositedM3: 0, flow: ff2.flow, flowMax: ff2.flowMax, pits: ff2.pits };
}

/** The pass library — the UI renders one "Simulate" button per entry. */
export const EROSION_PASSES = {
  fluvial:   { name: 'Fluvial (Stream Power)', desc: 'Hydraulic incision + selective diffusion + sediment routing. Run first.', fn: passFluvial },
  particles: { name: 'Particle Droplets (Hydraulic)', desc: 'Actual discrete physics water droplets with anti-tunneling bedload resistance. Carves natural gullies.', fn: passParticles },
  debris:    { name: 'Debris flows', desc: 'Slope failure: over-steep upper-flank faces collapse into gully channels and fan out at the toe.', fn: passDebris },
  hydraulic: { name: 'Hydraulic (Concentrated)', desc: 'Zero diffusion: deepens existing drainage into steep V-canyons.', fn: passHydraulic },
  rivers:    { name: 'Rivers only', desc: 'Only main riverbeds (A ≥ 4 m²) incise deeper and widen banks.', fn: passRivers },
  braid:     { name: 'Braided rivers', desc: 'Low-gradient reaches build sand bars and split flow into multi-thread channels.', fn: passBraid },
  micro:     { name: 'Micro rills (Detail)', desc: 'Fine 1.6–3.3 m dendritic branching rill network for razor-sharp micro-relief.', fn: passMicro },
  strata:    { name: 'Geological Strata (Terraces)', desc: 'Rock strata resistance: carves stepped benches and cliff ledges (Badlands/Canyon style).', fn: passStrata },
  diffuse:   { name: 'Hillslope diffusion', desc: 'Selective curvature smoothing: matures the surface without washing out cliffs.', fn: passDiffuse },
};
export const EROSION_PASS_ORDER = ['fluvial', 'particles', 'debris', 'micro', 'strata', 'hydraulic', 'rivers', 'braid', 'diffuse'];

/**
 * One-click erosion: runs the given pass stack (default:
 * fluvial → particles → debris → micro → strata) on the field, then refreshes
...
export async function runErosion({
  ...
  const passIds = stack || ['fluvial', 'particles', 'debris', 'micro', 'strata'];
  const base = {
