...
export class TerrainView {
  constructor(container, { onProbe, onFps } = {}) {
    ...
    // ---------------- sculpt state ----------------
    this.activeTool = 'view'; // 'view' | 'ridge' | 'dent' | 'smooth' | 'flatten' | 'chisel' | 'talus'
    this.brushRadius = 6.0;
    this.brushStrength = 0.35;
    this.brushFalloff = 'smooth';
    this.isSculpting = false;
    this.lastSculptPoint = new THREE.Vector3();
    this.targetElevation = 0;
    this.sculptCallbacks = {
      onStart: null,
      onStroke: null,
      onEnd: null,
    };

    // ---------------- probe & pointer events ----------------
    ...
    dom.addEventListener('pointerdown', (e) => {
      if (e.button === 0 && this.activeTool !== 'view') {
        ...
        if (this.terrainMesh) {
          ...
          if (hits.length > 0) {
            ...
            this.lastSculptPoint.copy(hits[0].point);
            this.targetElevation = hits[0].point.y;
            if (this.sculptCallbacks.onStart) {
              this.sculptCallbacks.onStart(hits[0].point, this.targetElevation);
            }
          }
        }
      }
    ...
    dom.addEventListener('pointermove', (e) => {
      ...
      // Update 3D brush cursor
      if (this.terrainMesh && this.activeTool !== 'view') {
        this.raycaster.setFromCamera(this.pointer, this.camera);
        const hits = this.raycaster.intersectObject(this.terrainMesh, false);
        if (hits.length > 0) {
          const hit = hits[0];
          this.brushRing.position.copy(hit.point);
          this.brushRing.position.y += 0.05; // avoid z-fight
          this.brushRing.scale.set(this.brushRadius, 1, this.brushRadius);
          this.brushRing.visible = true;

          if (this.isSculpting && this.sculptCallbacks.onStroke) {
            this.sculptCallbacks.onStroke(this.lastSculptPoint, hit.point, this.targetElevation);
            this.lastSculptPoint.copy(hit.point);
          }
        } else {
          this.brushRing.visible = false;
        }
    ...
    const finishSculpt = () => {
      if (this.isSculpting) {
        ...
        if (this.sculptCallbacks.onEnd) {
          this.sculptCallbacks.onEnd();
        }
      }
    };

    dom.addEventListener('pointerup', finishSculpt);
    dom.addEventListener('pointercancel', finishSculpt);
  ...
  setActiveTool(tool, color = '#4ade80') {
    this.activeTool = tool;
    if (this.brushRing) {
      this.brushRing.material.color.set(color);
      this.brushRing.visible = (tool !== 'view');
    }
  }

  setBrushRadius(r) {
    this.brushRadius = Math.max(0.5, r);
    if (this.brushRing) {
      this.brushRing.scale.set(this.brushRadius, 1, this.brushRadius);
    }
  }
  ...
  setSculptCallbacks(cbs) {
    this.sculptCallbacks = { ...this.sculptCallbacks, ...cbs };
  }

  /**
   * Fast real-time elevation update during brush stroke without disposing geometry.
   */
  updateHeights(h) {
    if (!this.terrainMesh || !this.probe) return;
    const pos = this.terrainMesh.geometry.attributes.position.array;
    const N = this.probe.N;
    for (let i = 0; i < N * N; i++) {
      pos[i * 3 + 1] = h[i];
    }
  ...
  /**
   * Update splat weights and terrain channel attributes after sculpting.
   */
  updateSplatAttributes(weights, channels, water) {
    if (!this.terrainMesh || !this.probe) return;
    const geo = this.terrainMesh.geometry;
    const N = this.probe.N;

    const splat = geo.attributes.splatA.array;
    const snow = geo.attributes.snowA.array;
    const slopeA = geo.attributes.slopeA.array;
    const wet = geo.attributes.wetA.array;
    const sed = geo.attributes.sedA.array;
    const waterA = geo.attributes.waterA.array;

    for (let i = 0; i < N * N; i++) {
      splat[i * 4 + 0] = weights[i * 5 + 0];
      splat[i * 4 + 1] = weights[i * 5 + 1];
      splat[i * 4 + 2] = weights[i * 5 + 2];
      splat[i * 4 + 3] = weights[i * 5 + 3];
      snow[i] = weights[i * 5 + 4];
      slopeA[i] = channels ? channels.slope[i] / (channels.slopeRef * 1.05) : 0;
      wet[i] = channels ? Math.min(1, channels.flowN[i] * 1.15) : 0;
      sed[i] = channels ? Math.min(1, channels.sedimentN[i] * 1.3 + channels.erosionN[i] * 0.35) : 0;
      waterA[i] = water ? water[i] : 0;
    }

    geo.attributes.splatA.needsUpdate = true;
    geo.attributes.snowA.needsUpdate = true;
    geo.attributes.slopeA.needsUpdate = true;
    geo.attributes.wetA.needsUpdate = true;
    geo.attributes.sedA.needsUpdate = true;
    geo.attributes.waterA.needsUpdate = true;
  }
