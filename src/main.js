...
import { defaults, SIZE, generateVolume } from "./field.js";
import { createRenderer } from "./renderer.js";
import { cameraEye, flyLook, flyMove, FLIGHT_KEYS } from "./camera.js";
const $=s=>document.querySelector(s), $$=s=>[...document.querySelectorAll(s)];
const params={...defaults};
const state={running:false, iterations:0, tool:"orbit", ready:false, rebuilding:false, busy:false, brush:null, renderedFrames:0, dirty:false};
...
function toast(text,d=3500){ const el=$("#toast"); el.textContent=text; el.classList.remove("hidden"); clearTimeout(toastTimer); toastTimer=setTimeout(()=>el.classList.add("hidden"),d); }
function format(id,v){
  ...
  if(["relief","radius","waterLevel"].includes(id)) return Number(v).toFixed(1)+" m";
...
function activateTab(name){
  ...
  $("#inspector-mode-label").textContent={terrain:"Terrain",sculpt:"Sculpt",erosion:"Erosion",water:"Water",fracture:"Fracture"}[name]||name;
...
async function rebuild(){
  ...
  try{
    const vol=generateVolume(params);
    renderer.upload(vol);
    state.iterations=0;
    $("#erosion-audit")?.classList.add("hidden");
    state.dirty=true; updateSim();
    toast("Fresh formation. Hydraulic erosion ready — footprint fixed to 1.2m, visible carving.");
...
function selectTool(tool){
  state.tool=tool; state.brush=null;
  ...
  if(tool!=="orbit") activateTab("sculpt");
}
$$(".sculpt-tool").forEach(el=>el.onclick=()=>selectTool(el.dataset.tool));
...
function updateSim(){
  ...
  const rl2=$("#run-label2"); if(rl2) rl2.textContent=state.running?"Pause":"Run";
  const pi=$("#run .play-icon"); if(pi) pi.textContent=state.running?"Ⅱ":"▶";
  $("#simulation-label").textContent=state.running?"Hydraulic + wind carving":state.iterations?"Landscape at rest":"Erosion ready";
  $("#sim-detail").textContent=state.running?`${["Rain","Runoff","River","Wind","Rock","Chem"][params.sourceMode]} · GPU` : state.iterations?"Paused":`Ready · footprint ${params.footprint}m`;
  ...
  $("#renderer-label").textContent=renderer?.gpu?"WebGL2 · GPU hydraulic":"WebGL2";
}
$("#run").onclick=()=>setRun(!state.running);
document.getElementById("run2")?.addEventListener("click",()=>setRun(!state.running));
...
$$("[data-source]").forEach(btn=>btn.onclick=()=>{
  ...
  $("#weather-description").textContent=["Rain → runoff & river","Concentrated runoff","River carves bed & banks","Wind abrasion → spires","Rock impact","Chemical dissolution"][params.sourceMode];
  updateSim(); toast(`Emitter: ${btn.textContent}. Footprint ${params.footprint}m now actually carves (fixed).`);
});
$("#audit-erosion").onclick=async()=>{
  ...
  try{
    ...
    out.textContent=`Active ${a.active}\nEroded ${a.eroded.toFixed(3)} m³\nDeposited ${a.deposited.toFixed(3)} m³\nCarried ${a.carried.toFixed(3)} m³\nRetired ${a.retired.toFixed(3)} m³\nSolid ${a.solid.toFixed(1)} / ${a.initialSolid.toFixed(1)} m³\nSteps ${a.steps}\nMass err ${a.massError===null?"N/A":a.massError.toExponential(2)}\n\nFIXED: cap 0.55*VOXEL_VOLUME (was 0.06), footprint 1.2m, thermal talus.`;
...
$("#view-mode").onclick=()=>{
  ...
  const sp=$("#view-mode span"); if(sp) sp.textContent=state.clay?"Clay":"Lit";
};
...
$("#export").onclick=async()=>{
  ...
  try{
    ...
    const header={format:"frontier-sdf",version:2,dimensions:SIZE,settings:{...params},camera:{...camera},iterations:state.iterations,model:"hydraulic-wind-fixed-v2"};
    ...
    const a=document.createElement("a"); a.href=url; a.download=`frontier-hydraulic-${params.seed}.frontier`; a.click();
    setTimeout(()=>URL.revokeObjectURL(url),30000);
    toast("Exported SDF + settings");
...
function bindCanvas(){
  ...
  const pick=async(x,y,apply,ver=strokeVer)=>{
    if(state.rebuilding||!state.ready||state.tool==="orbit") return;
    ...
    try{
      ...
      if(apply&&pt&&!state.busy&&!state.rebuilding){
        ...
        const count=lastStroke?Math.min(8,Math.max(1,Math.ceil(dist/spacing))):1;
        const from=lastStroke||pt;
        operation=(async()=>{
          if(dist<spacing&&lastStroke) return;
          for(let i=1;i<=count;i++){
            const dab=from.map((v,k)=>v+(pt[k]-v)*i/count);
            await renderer.sculpt(dab, params.radius, state.tool, {...params});
          }
  ...
  canvas.addEventListener("pointerdown",e=>{
    ...
    if(e.button===2){ flightKeys.clear(); navigation.looking=true; navigation.lockPending=true; try{ const lk=canvas.requestPointerLock?.(); Promise.resolve(lk).catch(()=>{}).finally(()=>{ navigation.lockPending=false; if(!navigation.looking&&document.pointerLockElement===canvas) document.exitPointerLock(); }); }catch{ navigation.lockPending=false; } }
    if(e.button===0&&!e.altKey&&state.tool!=="orbit") pick(e.clientX,e.clientY,true);
  });
  canvas.addEventListener("pointermove",e=>{
    ...
    } else if(state.tool!=="orbit"&&performance.now()-lastPick>80){ lastPick=performance.now(); pick(e.clientX,e.clientY,false); }
  });
  const end=e=>{
...
async function init(){
  ...
  try{
    ...
    resetCamera();
    renderer=await createRenderer(canvas,vol,fatal,uniforms(0),params);
    canvas=renderer.canvas||canvas;
    bindCanvas();
    state.ready=true;
    $("#loading").classList.add("hidden");
    $("#renderer-label").textContent=renderer.gpu?"WebGL2 · Hydraulic fixed":"WebGL2 · no GPU";
    if(!renderer.gpu){ $("#run").disabled=true; $("#step").disabled=true; toast(renderer.erosionError,12000); }
    requestAnimationFrame(frame);
    window.frontier={get backend(){return renderer.gpu?"webgl2-hydraulic-fixed":"webgl2"}, get iterations(){return state.iterations}, get settings(){return structuredClone(params)}, readVolume:()=>renderer.readVolume(), auditErosion:()=>renderer.auditErosion()};
    toast("Hydraulic SDF fixed: footprint 1.2m, erosion 0.55×VOXEL_VOLUME, wind spires. Run erosion to see valleys in 50 steps.",6000);
...
window.addEventListener("keydown",e=>{
  ...
  if(["1","2","3","4","5","6"].includes(e.key)) selectTool(["orbit","carve","add","smooth","ridge","dent"][Number(e.key)-1]);
});
