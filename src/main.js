...
import { defaults, presetData, SIZE, scenarios, setTier, TIERS, volumeVoxels } from "./core/constants.js";
...
function format(id, v) {
  ...
  if (id === "gully") return "×" + Number(v).toFixed(2);
...
async function rebuild() {
  ...
  try {
    setTier(params.tier || state.tier);
    state.tier = params.tier || state.tier;
    const vol = new Volume(SIZE);
    generateVolume(vol, params);
    renderer.upload(vol.data);
    $("#object-count").textContent = `1 OBJECT · ${(volumeVoxels() / 1e6).toFixed(1)}M VOX SDF · ${TIERS[state.tier].name.toUpperCase()}`;
    state.iterations = 0;
...
$("#tier")?.addEventListener("change", async e => {
  const next = e.target.value;
  if (!state.ready || next === state.tier) return;
  setRun(false);
  $("#loading").classList.remove("hidden");
  $("#loading strong").textContent = "Rebuilding volume";
  const span = $("#loading>span"); if (span) span.textContent = `${TIERS[next].name} · ${next === "high" ? "2.6M" : next === "draft" ? "0.55M" : "1.3M"} voxels…`;
  try {
    await operation;
    renderer.dispose();
    params.tier = next; state.tier = next;
    setTier(next);
    const vol = new Volume(SIZE);
    generateVolume(vol, params);
    renderer = await createRenderer(canvas, vol.data, fatal);
    // canvas event listeners survive the renderer swap — do NOT rebind
    state.iterations = 0; state.dirty = true;
    $("#object-count").textContent = `1 OBJECT · ${(volumeVoxels() / 1e6).toFixed(1)}M VOX SDF · ${TIERS[next].name.toUpperCase()}`;
    $("#loading").classList.add("hidden");
    updateSim();
    toast(`${TIERS[next].name} resolution active — same world, ${next === "high" ? "2.6" : next === "draft" ? "0.55" : "1.3"}M voxels.`, 4000);
...
function chooseScenario(id) {
  ...
  if (!sc || !state.ready) return;
  const keep = { preset: params.preset, seed: params.seed, relief: params.relief, strata: params.strata, roughness: params.roughness, footprint: params.footprint, grainSize: params.grainSize, waterLevel: params.waterLevel };
  Object.assign(params, sc, keep);
  params.tier = state.tier;
  $$("[data-source]").forEach(b => b.classList.toggle("active", Number(b.dataset.source) === params.sourceMode));
  $("#weather-description").textContent = { 0: "Rain → runoff → river. Wind carves spires.", 1: "Concentrated runoff — gullies.", 2: "River carves bed & banks.", 3: "Wind abrasion → spires & hoodoos.", 4: "Rockfall — impact craters & scree.", 5: "Chemical dissolution — karst." }[params.sourceMode];
  syncControls(); updateSim();
  $$("#scenario-row .preset").forEach(b => b.classList.toggle("active", b.dataset.scenario === id));
  toast(`${sc.name}: budgeted carve, swept channels, gully feedback on.`, 3200);
}
$$("#scenario-row .preset").forEach(el => el.onclick = () => chooseScenario(el.dataset.scenario));

$("#channel-toggle").onclick = () => {
  params.channel = params.channel ? 0 : 1;
  $("#channel-toggle").classList.toggle("on", !!params.channel);
};
...
Gully ×${(params.gully ?? 1.2).toFixed(2)} · swept carving ${params.channel ? "on" : "off"} · stability ${(params.stability ?? 0.5).toFixed(2)}
Footprint ${params.footprint} m → carve r ${Math.min(1.15, Math.max(0.5, params.footprint * 0.55)).toFixed(2)} m`;
...
$("#export").onclick = async () => {
  ...
  try {
    ...
    const header = { format: "frontier-sdf", version: 4, dimensions: SIZE.slice(), tier: state.tier, settings: { ...params }, camera: { ...camera }, iterations: state.iterations, model: "hybrid-stable-csg-v2" };
...
async function init() {
  syncControls(); updateSim();
  try {
    setTier(params.tier || "standard");
    state.tier = params.tier || "standard";
    const vol = new Volume(SIZE);
    generateVolume(vol, params);
    $("#object-count").textContent = `1 OBJECT · ${(volumeVoxels() / 1e6).toFixed(1)}M VOX SDF · ${TIERS[state.tier].name.toUpperCase()}`;
