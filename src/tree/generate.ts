...
import { buildSucculentOrgans, decorateSucculentMesh, mergeLeafMeshes } from '../plant/succulentMesher';
...
/**
 * Succulent pipeline: a cactus body is still made by the welded stem mesher;
 * rib relief and fleshy xeric organs are then added before validation.  This
 * keeps the same closed-manifold guarantee for the body while allowing plants
 * whose visible leaves are broad, curved organs rather than proxy tree foliage.
 */
function generateSucculent(params: TreeParams, options: { validate?: boolean; obstacleMeshes?: boolean }): GenerateResult {
  const s = { ...DEFAULT_SUCCULENT, ...(params.succulent ?? {}) };
  params.succulent = s;
  const t0 = now();
  const skeleton = buildSkeleton(params);
  const t1 = now();
  ...
  const primaries = buildRoots(skeleton, environment);
  const t2 = now();
  const built = buildMesh(skeleton);
  decorateSucculentMesh(built.mesh, s);
  mergeLeafMeshes(built.leaves, buildSucculentOrgans(params, s));
  let organTop = skeleton.height;
  for (let i = 1; i < built.leaves.positions.length; i += 3) organTop = Math.max(organTop, built.leaves.positions[i]);
  const t3 = now();
  ...
  return {
    ...
    summary: {
      stems: skeleton.stems.length,
      stemsPerLevel: skeleton.stemsPerLevel,
      leaves: built.leaves.count,
      height: organTop,
      treeScale: organTop,
      primaryRoots: primaries.filter((r) => !r.dropped).length,
      rootStems: built.stats.rootStems,
      obstacles: environment.count,
    },
    groundDepth: skeleton.groundDepth,
  };
