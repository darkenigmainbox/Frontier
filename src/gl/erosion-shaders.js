...
// Channel carving: the erosion demand is swept along the particle's travel
// segment (capsule) — moving flow incises continuous channels, not pit chains.
// All shaders are exported as *functions* so GLSL is generated at program
// construction with the active resolution tier baked in.

import { atlasGLSL, fullscreenVertex, header, uniformsGLSL } from "./common.js";
import { stampGLSL } from "../core/stamps.js";
import { SPLAT_SLICES, PARTICLE_SIZE, ATLAS_COLS, SIZE, MAX_SEGMENTS } from "../core/constants.js";

const H = () => header(atlasGLSL());
const U = uniformsGLSL;
const SEG = MAX_SEGMENTS;
...
// ----------------------------------------------------------------- event ----
// Per-particle contact + demand computation. The anti-runaway controller, the
// resolution-independent retreat-depth demand, and the swept-contact segment
// (prev → current surface projection) live here.
export const eventFragment = () => H() + U + `
uniform sampler2D anchors;                  // persistent carve anchors (xyz, valid)
...
layout(location=3) out vec4 nextAnchor;     // updated carve anchor
void main(){
  ivec2 uv=ivec2(gl_FragCoord.xy);
  int id=uv.y*${PARTICLE_SIZE[0]}+uv.x;
  contactEvent=vec4(0); exchangeEvent=vec4(0); contactNormal=vec4(0,1,0,0); nextAnchor=vec4(0);
  ...
  if(float(id)>=config.z || pos.w<0.) return;   // anchor stays invalid while dead
  nextAnchor=texelFetch(anchors,uv,0);          // default: carry the old anchor
  vec3 p=pos.xyz;
  ...
  if(kind==3.){ // wind abrasion
    ...
    // drainage networks self-organize into dendritic channels
    phi*=1.+extra.z*sampleVolume(terrain,c).g;
  }
  ...
  float depVox=process.z*max(0.,load-cap)*dt*dtScale*(0.6+1.6*concave);
  ...
  for(int z=-SPLAT_HALF;z<=SPLAT_HALF;z++)for(int y=-SPLAT_HALF;y<=SPLAT_HALF;y++)for(int x=-SPLAT_HALF;x<=SPLAT_HALF;x++){
  ...
  // persistent carve anchor: the splat sweeps this contact's demand from the
  // anchor to the current contact (capsule), then the anchor advances to c.
  nextAnchor=vec4(c,1.);
...
// (particle × capsule sub-center × z-slice); the demand is split evenly along
// the swept segment so the sum over sub-centers equals the contact demand.
export const splatVertex = () => H() + `
uniform sampler2D contacts, exchanges, normalsTex, species, anchors, metadata;
flat out float wet;
flat out vec4 spec;
...
flat out int slice;
void main(){
  int perParticle=${SPLAT_SLICES * SEG};
  int id=gl_InstanceID/perParticle;
  int rem=gl_InstanceID%perParticle;
  int k=rem/${SPLAT_SLICES};
  int off=rem%${SPLAT_SLICES}-${(SPLAT_SLICES - 1) / 2};
  ivec2 pUV=particleUV(id);
  vec4 c4=texelFetch(contacts,pUV,0);
  vec4 anchor=texelFetch(anchors,pUV,0);
  float kind=texelFetch(metadata,pUV,0).x;
  nrm=texelFetch(normalsTex,pUV,0);
  vec4 mixv=texelFetch(species,pUV,0)*vec4(0.4,0.07,1.,0);
  spec=mixv/max(dot(mixv,vec4(1)),1e-10);
  wet=nrm.w;
  // capsule sweep: from the persistent anchor to the current contact
  float segLen=anchor.w>0.5? distance(anchor.xyz,c4.xyz):0.;
  bool sweep=anchor.w>0.5 && kind!=4. && segLen>0.05 && segLen<5.5;
  float K=sweep? clamp(ceil(segLen/(0.6*c4.w)),1.,float(${SEG})) : 1.;
  vec3 from=sweep? anchor.xyz : c4.xyz;
  float t=(float(k)+1.)/K;
  vec3 center=mix(from,c4.xyz,t);
  exchange=texelFetch(exchanges,pUV,0)/K;
  contact=vec4(center,c4.w);
  int z=int(floor((center.z-LO.z)/CELL.z))+off;
  slice=z;
  if(c4.w<=0. || k>=int(K) || exchange.x+exchange.y<=0. || z<0 || z>=DIM.z){ gl_Position=vec4(-2,-2,0,1); return; }
  vec2 corner=vec2((gl_VertexID==1||gl_VertexID==2||gl_VertexID==4)?1.:-1., (gl_VertexID>=2&&gl_VertexID<=4)?1.:-1.);
  vec2 q=(center.xy-LO.xy)/CELL.xy + corner*(c4.w/CELL.xy + 1.2);
...
void main(){
  ...
  // moisture follows the exchange (gully self-organization signal)
  float wetAdd=6.*erodeVol/VOXEL_VOLUME + 0.5*depositVol/VOXEL_VOLUME;
  nextTerrain=vec4(dist, min(1., cur.g*0.995 + wetAdd), cur.b + depositVol, solid);
...
// Segment-aware: walks the same capsule sub-centers as the splat.
export const cargoFragment = () => H() + U + `
uniform sampler2D contacts, exchanges, acceptance, previousPositions, materials, anchors;
...
void main(){
  ...
  vec4 c4=texelFetch(contacts,uv,0), e=texelFetch(exchanges,uv,0), nrm=texelFetch(normalsTex,uv,0), anchor=texelFetch(anchors,uv,0);
  float pkind=texelFetch(metadata,uv,0).x;
  float accepted=0., acceptedDep=0.;
  vec4 newMat=vec4(0);
  if(c4.w>0. && e.x+e.y>0.){
    float segLen=anchor.w>0.5? distance(anchor.xyz,c4.xyz):0.;
    bool sweep=anchor.w>0.5 && pkind!=4. && segLen>0.05 && segLen<5.5;
    float K=sweep? clamp(ceil(segLen/(0.6*c4.w)),1.,float(${SEG})) : 1.;
    vec3 from=sweep? anchor.xyz : c4.xyz;
    for(int s=0;s<${SEG};s++){
      if(float(s)>=K) break;
      float t=(float(s)+1.)/K;
      vec3 c=mix(from,c4.xyz,t);
      float invK=1./K;
      ivec3 center=ivec3(floor((c-LO)/CELL));
      for(int z=-SPLAT_HALF;z<=SPLAT_HALF;z++)for(int y=-SPLAT_HALF;y<=SPLAT_HALF;y++)for(int x=-SPLAT_HALF;x<=SPLAT_HALF;x++){
        ivec3 q=center+ivec3(x,y,z);
        if(any(lessThan(q,ivec3(0)))||any(greaterThanEqual(q,DIM))) continue;
        float k=kernelWeight(worldAt(q),c,nrm.xyz,c4.w)*grainW(q, grainId(c), int(environment.w));
        if(k<=0.) continue;
        vec4 ground=fetchVoxel(terrain,q);
        vec2 w=exchangeW(ground,k);
        vec4 acc=texelFetch(acceptance, addr(q),0);
        accepted += e.x*invK*(w.x>0.? w.x/max(e.z,1e-9):0.)*acc.x;
        acceptedDep += e.y*invK*(w.y>0.? w.y/max(e.w,1e-9):0.)*acc.y;
        vec4 product;
