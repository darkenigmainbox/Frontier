...
// All size-dependent GLSL is generated *lazily* (function call at program
// construction) so resolution tiers only need a renderer rebuild.

import { SIZE, MIN, MAX, CELL, BAND, VOXEL_VOLUME, ATLAS_COLS, ATLAS, PARTICLE_SIZE, SPLAT_SLICES } from "../core/constants.js";

const f = (v) => (Number.isInteger(v) ? v.toFixed(1) : String(v));

export function atlasGLSL() {
  return /* glsl */ `
...
const int SPLAT_HALF=int(${(SPLAT_SLICES - 1) / 2});
...
uniform sampler2D terrain, positions, velocities, cargo, metadata, species, materials, impacts, normalsTex, previousPositions;
...
uniform vec4 extra;        // thermal, stability, gully, unused
`;
