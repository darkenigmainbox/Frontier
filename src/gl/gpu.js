...
import {
  SIZE, ATLAS, PARTICLE_SIZE, SPLAT_SLICES, MAX_SEGMENTS, BAND, VOXEL_VOLUME, carveRadius,
...
export class GPUErosion {
  constructor(gl) {
    ...
    // Resolution tier is baked into the GLSL at construction (lazily generated).
    ...
    this.anchors = [this.makeTex(...PARTICLE_SIZE), this.makeTex(...PARTICLE_SIZE)]; // persistent carve anchors
    this.anchorIdx = 0;
  ...
  // Release every GL resource — used when switching resolution tiers
  // (textures/programs are rebuilt with the new baked constants).
  dispose() {
    const gl = this.gl;
    for (const key of Object.keys(this)) {
      const v = this[key];
      if (Array.isArray(v)) { for (const t of v) if (gl.isTexture(t)) gl.deleteTexture(t); }
      else if (gl.isTexture(v)) gl.deleteTexture(v);
    }
    for (const name of Object.keys(this.progs)) gl.deleteProgram(this.progs[name].handle);
  ...
  upload(data) {
    const a = new Float32Array(this.width * this.height * 4);
    let mass = 0;
    for (let z = 0; z < SIZE[2]; z++) for (let y = 0; y < SIZE[1]; y++) {
      const src = (z * SIZE[1] + y) * SIZE[0] * 4;
      const dst = ((Math.floor(z / 16) * SIZE[1] + y) * this.width + (z % 16) * SIZE[0]) * 4;
      a.set(data.subarray(src, src + SIZE[0] * 4), dst);
    }
    this.volumeIdx = 0;
    this.uploadTex(this.volume, this.width, this.height, a);
    ...
    for (let i = 0; i < 2; i++) {
      ...
      this.target([this.materials[i]], this.width, this.height);
    ...
    for (let i = 0; i < 2; i++) this.uploadTex(this.anchors[i], ...PARTICLE_SIZE, empty);
    this.anchorIdx = 0;
  ...
  uniformValues(params) {
    return {
      ...
      extra: [params.thermal ?? 0.35, params.stability ?? 0.5, params.gully ?? 1.2, 0],
    };
  }
  advance(params) {
    const gl = this.gl;
    ...
    // 2) event: contacts + bounded demands (anti-runaway) + carve-anchor advance
    this.pass("event", [this.contacts, this.exchanges, this.normals, this.anchors[1 - this.anchorIdx]], ...PARTICLE_SIZE, {
      terrain: oldVol, positions: this.positions[motion], previousPositions: prevPos, velocities: this.velocities[motion], cargo: oldCargo, metadata: this.metas[motion], species: oldSpecies, impacts: this.impacts, materials: oldMat, anchors: this.anchors[this.anchorIdx],
    }, vals);

    // 3) splat: scatter demands additively into the atlas (capsule sub-centers × slices)
    this.target([this.requests, this.speciesReq], this.width, this.height);
    gl.clearColor(0, 0, 0, 0); gl.clear(gl.COLOR_BUFFER_BIT);
    this.use(this.progs.splat, {
      terrain: oldVol, contacts: this.contacts, exchanges: this.exchanges, normalsTex: this.normals, species: oldSpecies, anchors: this.anchors[this.anchorIdx], metadata: this.metas[motion],
    ...
    gl.drawArraysInstanced(gl.TRIANGLES, 0, 6, this.activeCount * SPLAT_SLICES * MAX_SEGMENTS);
    gl.disable(gl.BLEND);

    // 4) apply: materialize with per-voxel caps + armor accumulation
    this.pass("apply", [this.volumes[nextVol], this.accept, this.materials[nextMat]], this.width, this.height, {
      terrain: oldVol, requests: this.requests, materials: oldMat, speciesRequests: this.speciesReq,
    });

    // 5) cargo: settle accepted amounts into particle sediment (capsule-aware)
    this.pass("cargo", [this.cargos[nextCargo], this.species[nextCargo]], ...PARTICLE_SIZE, {
      terrain: oldVol, contacts: this.contacts, exchanges: this.exchanges, acceptance: this.accept, positions: this.positions[motion], previousPositions: prevPos, cargo: oldCargo, metadata: this.metas[motion], species: oldSpecies, impacts: this.impacts, materials: oldMat, normalsTex: this.normals, anchors: this.anchors[this.anchorIdx],
    }, vals);

    // 6) redistancing ×2 (alternating sweeps) — keeps the SDF valid & crisp
    this.pass("distance", [this.volumes[this.volumeIdx]], this.width, this.height, { terrain: this.volumes[nextVol] }, { sweepSign: 0 });
    this.pass("distance", [this.volumes[nextVol]], this.width, this.height, { terrain: this.volumes[this.volumeIdx] }, { sweepSign: 1 });
    this.volumeIdx = nextVol;
    this.pass("clamp", [this.volumes[1 - this.volumeIdx]], this.width, this.height, { terrain: this.volumes[this.volumeIdx] });
    ...
    if (this.tick % 3 === 0 && (params.thermal ?? 0) > 0) {
      ...
      this.pass("thermal", [this.volumes[nv], this.materials[nm]], this.width, this.height, {
    ...
    this.motionIdx = motion; this.cargoIdx = nextCargo; this.anchorIdx = 1 - this.anchorIdx; this.tick++;
  ...
  async sculpt(point, radius, tool, strength = 0.5, stampType = 0, stampMode = 0, redistances = 6) {
    ...
    this.pass("sculpt", [this.volumes[other], this.materials[otherMat]], this.width, this.height, {
    ...
    for (let i = 0; i < redistances; i++) {
      this.pass("distance", [this.volumes[1 - this.volumeIdx]], this.width, this.height, { terrain: this.volumes[this.volumeIdx] }, { sweepSign: i % 2 });
      this.volumeIdx = 1 - this.volumeIdx;
    }
    this.pass("clamp", [this.volumes[1 - this.volumeIdx]], this.width, this.height, { terrain: this.volumes[this.volumeIdx] });
    this.volumeIdx = 1 - this.volumeIdx;
  ...
  async readVolume() {
    await this.complete();
    const atlas = this.read(this.volume, this.width, this.height), a = new Float32Array(SIZE[0] * SIZE[1] * SIZE[2] * 4);
    for (let z = 0; z < SIZE[2]; z++) for (let y = 0; y < SIZE[1]; y++) {
      const src = ((Math.floor(z / 16) * SIZE[1] + y) * this.width + (z % 16) * SIZE[0]) * 4;
