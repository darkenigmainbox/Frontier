...
import {
  ...
  THIN_GUARD, STALL_SPEED, STALL_DEPOSIT, carveRadius, BAND,
} from "./constants.js";
...
export class Solver {
  constructor(vol, params) {
    ...
    this.reqWet = new Float32Array(n);
  }
  ...
  events(dt) {
    ...
    for (let i = 0; i < this.N; i++) {
      ...
      depositVox = depositionRate * excess * dt * dtScale * (0.6 + 1.2 * concave);
  ...
  applyRequest(c, n, r, pid, erode, deposit) {
    ...
    for (let j = 0; j < idxs.length; j++) {
      ...
      this.reqWet[vi] += 0.02 / idxs.length;
    }
