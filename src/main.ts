...
import { SucculentParams } from './plant/succulentParams';
...
const tabSucculent = el('button', '', 'Xeric');
...
function syncTabs(): void {
  ...
  for (const [b] of tabMap) {
    const hide = grass ? treeTabs.has(b) || succulentTabs.has(b) : succulent ? grassTabs.has(b) : grassTabs.has(b) || succulentTabs.has(b);
    b.style.display = hide ? 'none' : '';
  }
  const active = tabMap.find(([b]) => b.classList.contains('on'))?.[0];
  if (succulent && active !== tabSucculent) showTab(tabSucculent);
  else if (!active || active.style.display === 'none') showTab(grass ? tabGrass : tabBotany);
  inspectorBadge.textContent = grass ? 'grass · welded' : succulent ? 'xeric · welded' : 'tree · welded';
}
showTab(tabBotany);
...
worker.onmessage = (ev: MessageEvent<WorkerResponse>) => {
  ...
  if (msg.type === 'generated') {
    ...
    const material = isSucculent(params) && params.succulent
      ? (params.succulent.form === 'agave' || params.succulent.form === 'aloe' ? 'waxy' : 'cactus')
      : 'bark';
    viewer.setTree(buffers, leaves, msg.summary.height, material);
...
const GROUP_ACCENT: Record<string, string> = {
  ...
  Savanna: '#e5d33a',
  Desert: '#ffb454',
  'Desert · cacti & succulents': '#ffb454',
...
function speciesMeta(name: string): string {
  ...
  if (isSucculent(p) && p.succulent) return `${p.succulent.form} · ${p.botany.scale} m · welded body`;
  const b = p.botany;
...
function buildSucculentPage(): void {
  ...
  const formNames: [SucculentParams['form'], string][] = [
    ['saguaro', 'Saguaro arms'],
    ['barrel', 'Ribbed barrel'],
    ['prickly-pear', 'Cladode pads'],
    ['agave', 'Agave rosette'],
    ['aloe', 'Aloe rosette'],
  ];
  const sForm = section('Species form', page, { hint: 'xeric detail' });
  select(sForm, 'Surface / organ set', formNames.map(([value, label]) => ({ value, label })), () => S().form, (v) => (S().form = v as SucculentParams['form']), onChange);
  const sRib = section('Cactus ribs', page, { hint: S().ribCount ? `${S().ribCount} ribs` : 'rosette only' });
  stepper(sRib, 'Rib count', () => S().ribCount, (v) => (S().ribCount = v), { min: 4, max: 32, integer: true, title: 'Longitudinal ribs on the cactus body' }, onChange);
  slider(sRib, 'Rib relief', () => S().ribDepth, (v) => (S().ribDepth = v), { min: 0, max: 0.12, step: 0.005, unit: '× r', title: 'Radial relief relative to the body radius' }, onChange);
  slider(sRib, 'Valley softness', () => S().ribSoftness, (v) => (S().ribSoftness = v), { min: 0.04, max: 0.9, step: 0.02, title: 'Keeps valleys soft and botanical rather than gear-like' }, onChange);
  const sOrg = section('Fleshy organs', page, { hint: `${S().rosetteCount} organs · ${S().spineCount} areoles` });
  stepper(sOrg, 'Organ count', () => S().rosetteCount, (v) => (S().rosetteCount = v), { min: 0, max: 80, integer: true, title: 'Curved leaves or prickly-pear cladodes grown from the crown' }, onChange);
  stepper(sOrg, 'Areoles / spines', () => S().spineCount, (v) => (S().spineCount = v), { min: 0, max: 180, integer: true, title: 'Small seeded spine cards on ribbed cactus bodies' }, onChange);
  slider(sOrg, 'Spine length', () => S().spineLength, (v) => (S().spineLength = v), { min: 0.005, max: 0.12, step: 0.001, unit: 'm' }, onChange);
  slider(sOrg, 'Length', () => S().rosetteLength, (v) => (S().rosetteLength = v), { min: 0.06, max: 1.6, step: 0.01, unit: 'm' }, onChange);
  slider(sOrg, 'Width', () => S().rosetteWidth, (v) => (S().rosetteWidth = v), { min: 0.02, max: 0.8, step: 0.01, unit: 'm' }, onChange);
  slider(sOrg, 'Lean', () => S().rosetteLean, (v) => (S().rosetteLean = v), { min: 0, max: 78, step: 1, unit: '°' }, onChange);
  slider(sOrg, 'Outer droop', () => S().rosetteDroop, (v) => (S().rosetteDroop = v), { min: 0, max: 70, step: 1, unit: '°' }, onChange);
  slider(sOrg, 'Variation', () => S().rosetteVariation, (v) => (S().rosetteVariation = v), { min: 0, max: 0.5, step: 0.02, unit: '±' }, onChange);
  slider(sOrg, 'Pad bulge', () => S().padBulge, (v) => (S().padBulge = v), { min: 0, max: 0.2, step: 0.01, unit: 'm' }, onChange);
  page.append(Object.assign(el('p', 'note'), {
    textContent: 'Cactus ribs are part of the body surface, not a floating overlay. Agave and aloe leaves are curved five-point organs with per-leaf wind data; prickly-pear pads use the same seeded crown generator. Change the seed to review another specimen before exporting.',
  }));
}
