...
const CACTUS = new THREE.Color(0x477457);
const WAXY = new THREE.Color(0x5d8f65);
...
export class Viewer {
  ...
  setTree(buffers: GpuBuffers, leaves: LeafMesh | null, height: number, style: 'bark' | 'cactus' | 'waxy' = 'bark'): void {
    this.clearTree();
    ...
    const mat = this.makeBarkMaterial(style);
    ...
    if (leaves && leaves.count > 0) {
      ...
      const lmat = this.makeLeafMaterial(style);
  ...
  frame(): void {
    ...
    const branchPos = this.branchMesh ? (this.branchMesh.geometry.getAttribute('position').array as Float32Array) : null;
    const leafPos = this.leafMesh ? (this.leafMesh.geometry.getAttribute('position').array as Float32Array) : null;
    const pos = branchPos && leafPos
      ? Float32Array.from([...branchPos, ...leafPos])
      : branchPos ?? leafPos;
    let cx = 0;
  ...
  private makeBarkMaterial(style: 'bark' | 'cactus' | 'waxy' = 'bark'): THREE.MeshStandardMaterial {
    const mat = new THREE.MeshStandardMaterial({
      color: style === 'cactus' ? CACTUS : style === 'waxy' ? WAXY : BARK,
      roughness: style === 'bark' ? 0.92 : 0.78,
      metalness: 0.0,
  ...
  private makeLeafMaterial(style: 'bark' | 'cactus' | 'waxy' = 'bark'): THREE.MeshStandardMaterial {
    const mat = new THREE.MeshStandardMaterial({
      color: style === 'cactus' ? new THREE.Color(0x9cb77a) : style === 'waxy' ? new THREE.Color(0x4f874c) : LEAF,
      roughness: style === 'bark' ? 0.75 : 0.58,
      metalness: 0,
      side: THREE.DoubleSide,
    });
