...
// The remaining reference-panel entities are project presentation state. They are deliberately kept here, beside
// the weather settings, instead of being hidden in the editor host: terrain, camera and post controls can therefore
// be serialized or driven by another Project Zero front end without inventing a second celestial registry.
struct TerrainPanelSettings
{
    bool     Enabled   = true;
    float    Size      = 180.0f;  // [m]
    float    Height    = 12.0f;   // [m]
    float    Frequency = 1.2f;    // [1/m]
    uint32_t Seed      = 417u;
    uint32_t Resolution = 256u;   // [samples]
    bool     Wireframe = false;
    bool     CastShadow = true;
    float    Position[3] = { 0.0f, 0.0f, 0.0f }; // [m]
};

struct CameraPanelSettings
{
    bool  Enabled          = true;
    float FieldOfView      = 72.0f;  // [deg]
    float NearClip         = 0.1f;   // [m]
    float FocusDistance    = 8.0f;   // [m]
    float Yaw              = 35.0f;  // [deg]
    float Pitch            = -4.0f;  // [deg]
    float Roll             = 0.0f;   // [deg]
    bool  Smooth            = true;
};

struct PostPanelSettings
{
    bool  Enabled          = true;
    float Exposure         = 1.05f;  // [EV]
    uint32_t ToneMap       = 0u;    // ACES, Reinhard, Filmic, AgX
    float Bloom            = 0.55f; // [x]
    float Grain            = 0.0f;  // [x]
    bool  LensFlare        = true;
};
...
public:
    ...
    TerrainPanelSettings Terrain{};
    CameraPanelSettings  Camera{};
    PostPanelSettings    Post{};
