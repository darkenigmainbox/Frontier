                                          ...
                                          const ObserverFrame& Observer) const noexcept
{
    ...
    //    Reference `main()` line 1255: `length(uSunUV*vec2(uRes.y/uRes.x,1.))` — the gate is measured in
    //    height units, so the sun's horizontal offset is divided by the viewport aspect first. Without the
    //    division a wide frame keeps the flare alive too far past the left/right edges.
    const float Aspect  = Observer.AspectRatio > 1e-6f ? Observer.AspectRatio : 1.0f;
    const float GateU   = SunU / Aspect;
    const float InFrame = 1.0f - SmoothStep(1.25f, 2.2f, std::sqrt(GateU * GateU + SunV * SunV));
