...
constexpr float kTintTerrain[3]    = { 0.561f, 0.702f, 0.420f };   // #8fb36b
constexpr float kTintCamera[3]     = { 0.204f, 0.780f, 0.349f };   // #34c759
constexpr float kTintPost[3]       = { 1.000f, 0.541f, 0.396f };   // #ff8a65
...
const char* CelestialEntityName(CelestialEntity Entity) noexcept
{
    ...
    case CelestialEntity::VolumetricClouds:return "Clouds";
    ...
    case CelestialEntity::Terrain:        return "Height Field";
    case CelestialEntity::Camera:         return "Camera";
    case CelestialEntity::PostProcess:    return "Post Process";
    default:                              return "?";
    }
}
...
const char* CelestialEntityKind(CelestialEntity Entity) noexcept
{
    ...
    case CelestialEntity::VolumetricClouds:return "Volumetric Clouds";
    ...
    case CelestialEntity::Terrain:        return "Terrain";
    case CelestialEntity::Camera:         return "Perspective Camera";
    case CelestialEntity::PostProcess:    return "Component · Camera";
    default:                              return "?";
    }
}
...
void CelestialSequence::ApplyTo(VisibilityRaster& Raster, const CelestialBudget& Budget) const noexcept
{
    ...
    // Height and aerial fog are analytic media rather than box volumes. Keep both controls in the same
    // settings record so the CPU preview does not silently ignore the two fog entities when their outliner eye
    // is toggled. The GPU sky still receives the local volume fields above; this is the GI-off finite-segment
    // contribution that belongs in the visibility raster.
    Settings.Fog = Fog;
    Settings.Fog.HeightEnabled = Enabled && Shown[static_cast<uint32_t>(CelestialEntity::HeightFog)]
                              && Fog.HeightEnabled;
    Settings.Fog.AerialEnabled = Enabled && Shown[static_cast<uint32_t>(CelestialEntity::AtmosphericFog)]
                               && Fog.AerialEnabled;
    Settings.Wind = Wind;
...
SkyConstantRecord CelestialSequence::PackSkyRecord() const noexcept
{
    ...
    PackSkyFog(Record, Fog,
               Enabled && Shown[static_cast<uint32_t>(CelestialEntity::HeightFog)],
               Enabled && Shown[static_cast<uint32_t>(CelestialEntity::AtmosphericFog)]);
    return Record;
}
...
uint32_t CelestialSequence::AppendRoster(EditorInstance* Instances, uint32_t Written, uint32_t Capacity) const noexcept
{
    ...
    for (uint32_t I = 0; I < kCelestialEntityCount; ++I)
    {
        ...
        Row.Category = Entity == CelestialEntity::Sun ? EditorInstanceCategory::Light
                     : Entity == CelestialEntity::Camera ? EditorInstanceCategory::Camera
        ...
        Row.Dynamic = Entity == CelestialEntity::Sun || Entity == CelestialEntity::CloudLayer
                   || Entity == CelestialEntity::VolumetricClouds || Entity == CelestialEntity::Wind
                   || Entity == CelestialEntity::Precipitation || Entity == CelestialEntity::Stars
                   || Entity == CelestialEntity::Moons;
        switch (Entity)
        {
        ...
        case CelestialEntity::VolumetricClouds:
        ...
        case CelestialEntity::Terrain:        CopyTint(Row.Tint, kTintTerrain);    break;
        case CelestialEntity::Camera:         CopyTint(Row.Tint, kTintCamera);     break;
        case CelestialEntity::PostProcess:    CopyTint(Row.Tint, kTintPost);       break;
...
void CelestialSequence::BuildSheet(CelestialEntity Entity, EditorSheet& Sheet) const noexcept
{
    ...
    case CelestialEntity::Terrain:
    {
        EditorPropertyGroup& Shape = OpenGroup(Sheet, "Height Field");
        Push(Shape, MakeSwitch("Enabled", Terrain.Enabled));
        Push(Shape, MakeSlider("Size", 16.0f, 2000.0f, Terrain.Size, 0, "m"));
        Push(Shape, MakeSlider("Height", 0.0f, 300.0f, Terrain.Height, 1, "m"));
        Push(Shape, MakeSlider("Frequency", 0.05f, 8.0f, Terrain.Frequency, 2, "/m"));
        Push(Shape, MakeSlider("Seed", 0.0f, 10000.0f, static_cast<float>(Terrain.Seed), 0, ""));
        Push(Shape, MakeSlider("Resolution", 32.0f, 1024.0f, static_cast<float>(Terrain.Resolution), 0, ""));
        Push(Shape, MakeSwitch("Wireframe", Terrain.Wireframe));
        Push(Shape, MakeSwitch("Cast Shadow", Terrain.CastShadow));
        Push(Shape, MakeAxes("Position", Terrain.Position, 1.0f));
        break;
    }
    case CelestialEntity::Camera:
    {
        EditorPropertyGroup& Lens = OpenGroup(Sheet, "Perspective");
        Push(Lens, MakeSwitch("Active", Camera.Enabled));
        Push(Lens, MakeSlider("Field of View", 20.0f, 120.0f, Camera.FieldOfView, 1, "deg"));
        Push(Lens, MakeSlider("Near Clip", 0.01f, 10.0f, Camera.NearClip, 2, "m"));
        Push(Lens, MakeSlider("Focus Distance", 0.1f, 1000.0f, Camera.FocusDistance, 1, "m"));
        EditorPropertyGroup& Orbit = OpenGroup(Sheet, "Orientation");
        Push(Orbit, MakeSlider("Yaw", -180.0f, 180.0f, Camera.Yaw, 1, "deg"));
        Push(Orbit, MakeSlider("Pitch", -89.0f, 89.0f, Camera.Pitch, 1, "deg"));
        Push(Orbit, MakeSlider("Roll", -180.0f, 180.0f, Camera.Roll, 1, "deg"));
        Push(Orbit, MakeSwitch("Smooth", Camera.Smooth));
        break;
    }
    case CelestialEntity::PostProcess:
    {
        EditorPropertyGroup& Tone = OpenGroup(Sheet, "Tone Map");
        Push(Tone, MakeSwitch("Enabled", Post.Enabled));
        static const char* const Curves[] = { "ACES", "Reinhard", "Filmic", "AgX" };
        Push(Tone, MakeSelect("Curve", Curves, 4u, Post.ToneMap));
        Push(Tone, MakeSlider("Exposure", -8.0f, 8.0f, Post.Exposure, 2, "EV"));
        EditorPropertyGroup& Effects = OpenGroup(Sheet, "Effects");
        Push(Effects, MakeSlider("Bloom", 0.0f, 2.0f, Post.Bloom, 2, "x"));
        Push(Effects, MakeSlider("Grain", 0.0f, 1.0f, Post.Grain, 2, "x"));
        Push(Effects, MakeSwitch("Lens Flare", Post.LensFlare));
        break;
    }
    default:
        break;
...
void CelestialSequence::ApplySheet(CelestialEntity Entity, const EditorSheet& Sheet) noexcept
{
    ...
    case CelestialEntity::LensFlare:
        ...
        break;
    case CelestialEntity::Terrain:
        Terrain.Enabled    = ReadSwitch(Sheet, "Enabled", Terrain.Enabled);
        Terrain.Size       = ReadSlider(Sheet, "Size", Terrain.Size);
        Terrain.Height     = ReadSlider(Sheet, "Height", Terrain.Height);
        Terrain.Frequency  = ReadSlider(Sheet, "Frequency", Terrain.Frequency);
        Terrain.Seed       = static_cast<uint32_t>(ReadSlider(Sheet, "Seed", static_cast<float>(Terrain.Seed)));
        Terrain.Resolution = static_cast<uint32_t>(ReadSlider(Sheet, "Resolution", static_cast<float>(Terrain.Resolution)));
        Terrain.Wireframe  = ReadSwitch(Sheet, "Wireframe", Terrain.Wireframe);
        Terrain.CastShadow = ReadSwitch(Sheet, "Cast Shadow", Terrain.CastShadow);
        ReadAxes(Sheet, "Position", Terrain.Position);
        break;
    case CelestialEntity::Camera:
        Camera.Enabled       = ReadSwitch(Sheet, "Active", Camera.Enabled);
        Camera.FieldOfView   = ReadSlider(Sheet, "Field of View", Camera.FieldOfView);
        Camera.NearClip      = ReadSlider(Sheet, "Near Clip", Camera.NearClip);
        Camera.FocusDistance = ReadSlider(Sheet, "Focus Distance", Camera.FocusDistance);
        Camera.Yaw           = ReadSlider(Sheet, "Yaw", Camera.Yaw);
        Camera.Pitch         = ReadSlider(Sheet, "Pitch", Camera.Pitch);
        Camera.Roll          = ReadSlider(Sheet, "Roll", Camera.Roll);
        Camera.Smooth        = ReadSwitch(Sheet, "Smooth", Camera.Smooth);
        break;
    case CelestialEntity::PostProcess:
        Post.Enabled    = ReadSwitch(Sheet, "Enabled", Post.Enabled);
        Post.ToneMap    = ReadSelect(Sheet, "Curve", Post.ToneMap);
        Post.Exposure   = ReadSlider(Sheet, "Exposure", Post.Exposure);
        Post.Bloom      = ReadSlider(Sheet, "Bloom", Post.Bloom);
        Post.Grain      = ReadSlider(Sheet, "Grain", Post.Grain);
        Post.LensFlare  = ReadSwitch(Sheet, "Lens Flare", Post.LensFlare);
        break;
    default:
        break;
    }
